...
test('children can be an array (for preact support)', () => {
  ...
  render(<Toggler>{[childSpy]}</Toggler>)
  expect(childSpy).toHaveBeenLastCalledWith(
    expect.objectContaining({on: false}),
  )
})
...
test('onToggle gets called in controlled prop scenario', () => {
  const spy = jest.fn()
  const {rerender} = setup({on: false, onToggle: spy})
...
test('onToggle gets called on internal state change in controlled prop scenario', () => {
  const spy = jest.fn()
  const {setOn, setOff, rerender} = setup({on: false, onToggle: spy})
  setOff()
  expect(spy).not.toHaveBeenCalled()
  setOn()
  rerender({on: true})
...
function setup({children = () => <div />, ...props} = {}) {
  ...
  const utils = render(<Toggler {...props}>{childSpy}</Toggler>)
  const rerender = newProps =>
    utils.rerender(<Toggler {...newProps}>{childSpy}</Toggler>)
  return {childSpy, ...utils, rerender, ...renderArg}
}
